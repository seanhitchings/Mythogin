...
function renderHighlightedCard(item, gridEl, cfg) {
  ...
  function filterHighlighted(items, cfg) {
    cfg = cfg || {};
    return (items || [])
      .filter(function (it) { return String((it && it.category) || "") === cfg.category; })
      .filter(function (it) { return !cfg.type || String((it && it.type) || "") === cfg.type; })
      .filter(function (it) { return coerceBool(it && it.highlighted); });
  }
    ...
    loadSearchIndexItems()
      .then(function (items) {
        grids.forEach(function (grid) {
          ...
          var picks = filterHighlighted(items, cfg);
